...
const KualitasDetail = () => {
  ...
  const deleteData = async () => {
    await axios
      .delete(HOST + "/marketing/kualitasdetail/delete/" + data.id, {
        headers: {
          "ngrok-skip-browser-warning": "true",
          Authorization: getCookie("admin_auth"),
        },
  ...
  useEffect(() => {
    if (getActionButton === "update" && data.length !== 0) {
      navigate("/dashboard/master/kualitas-detail/update");
  ...
  return pageLoading ? (
    ...
    <div>
      ...
      <div className="m-2 md:m-10 mt-24 px-2 py-10 md:p-10 bg-white rounded-3xl">
        <Header title="Kualitas Detail" />
        <div className="mb-4 -mt-4">
          <button
            className="bg-blue-700 rounded-xl text-white px-4 py-2"
            onClick={() => {
              navigate("/dashboard/master/kualitas-detail/tambah");
            }}
          >
